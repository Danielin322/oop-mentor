...
export class GeminiService {
  ...
  public async generateStudyCardContent(term: string, language: string) {
    const res = await fetch("/api/study-card", {
      method: "POST",
    ...
    if (!res.ok) {
      throw new Error("Study card request failed");
    }
  ...
  //   return this.safeJsonParse<CodingLabChallenge>(
  //     response.text,
  ...
  //     const challengeText = this.formatChallengeForEvaluation(
  //       challengeOrDescription,
  //     );
  ...
  //     const parsed = this.safeJsonParse<any>(
  //       response.text,
  ...
  //       failingCases: Array.isArray(parsed.failingCases)
  //         ? parsed.failingCases.map(String)
  //         : undefined,
  //       improvements:
  //         parsed.improvements != null ? String(parsed.improvements) : undefined,
  //     };

  //     return result;
  //   }

  // private formatChallengeForEvaluation(
  //   challengeOrDescription: CodingLabChallenge | string,
  // ): string {
